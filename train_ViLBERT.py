...
from lib.vanilla_utils import CONFIG
LOG_INTERVAL = 50
VAL_INTERVAL = 1000
...
def main(args):
    ...
    with open(ctxdb_path, 'r') as f:
        ctxdb = json.load(f)
    # Build dataloaders
    trn_dataset = ViLBERTDataset(refdb, ctxdb, split='train', roi_per_img=CONFIG['ROI_PER_IMG'])
    val_dataset = ViLBERTDataset(refdb, ctxdb, split='val', roi_per_img=CONFIG['ROI_PER_IMG'])
    ...
    with open("./config/vilbert_tasks.yml", "r") as f:
    ...
    for i, task_id in enumerate(args.tasks.split("-")):
        task = "TASK" + task_id
        task_batch_size[task] = CONFIG['BATCH_SIZE']
        task_datasets_train[task] = trn_dataset
        task_dataloader_train[task] = trn_loader
        task_datasets_val[task] = val_dataset
        task_dataloader_val[task] = val_loader
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_name",
        default="finetune_from_multi_task_model",
        type=str,
    ...
    parser.add_argument(
        "--tasks", default="9", type=str, help="1-2-3... training task separate by -"
    )
    parser.add_argument(
        "--freeze",
        default=-1,
